...
where the $\beta_j$ ' are unknow parameters or coefficients.
...
where $f_1$ and $f_2$ are smooth funcyions of covariated $x_1$ and $x_2$. The log is an example of a link function. 
...
gam_model_inter <- gam(y ~ s(x, by = factor(interaction_term)))
...
for(i in 1:k)
{
  ...
  gam_model_for_folds <- gam(data = train_data, y ~ s(x))
