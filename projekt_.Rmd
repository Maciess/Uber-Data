...
data_for_model <- data.frame(x=x, y=y)
resulting_folds <- split_data_into_folds(data_for_model, k)

dfWithMetrics <- data.frame(RMSE = c(), R_squared = c())
...
for(i in 1:k)
{
  ...
  gam_model_for_folds <- mgcv::gam(data = train_data, y ~ s(x))
  predicted <- predict(gam_model_for_folds, newdata = test_set)
  
  RMSE <- Metrics::rmse(actual = test_set$y, predicted = predicted)
  R_squared <- summary(gam_model_for_folds)$r.sq
  dfWithMetrics <- rbind(dfWithMetrics, c(RMSE, R_squared))  
