...
We added two columns to the raw data - Service time, which shows the duration of the service in minutes. To do this, we first standardized the format of the start and end date. Second added column is Daytime column, which indicates whether the service took place in the morning, evening, etc., we believe this variable can be significant for modelling. The short summary of data after cleansing is presented below.
...
First, we constructed GAM model using only miles as explanatory variable. In our second model we combined miles and hours as explanatory variables. The best model for these two variables is the model where we used the miles ad smoothing term.
...
for(i in 1:k)
{
  ...
  dfWithMetrics <- rbind(dfWithMetrics, c(RMSE, R_squared, i, "Simplest lm")) 
  ...
  dfWithMetrics <- rbind(dfWithMetrics, c(RMSE, R_squared, i, "Gam model"))  
  
  
  #What metrics we want to calculate?
}
...
```{r plot-metrics}
colnames(dfWithMetrics) <- c("RMSE", "R_squared", "Fold_number", "Model")

RMSE_plot <- ggplot(data = dfWithMetrics, aes(x = Fold_number, y = RMSE, color = Model, shape = Model))+
  ggtitle("RMSE per fold")+
  geom_point()+
  geom_hline(yintercept=mean(RMSE), linetype="dashed", color = "red")


R_squared_plot <- ggplot(data = dfWithMetrics, aes(x = Fold_number, y = R_squared, color = Model, shape = Model))+
  ggtitle(TeX("$R^2$ per fold"))+
  geom_point()+
  geom_line(aes(y = ave(R_squared, Model, FUN = median)))

grid.arrange(RMSE_plot, R_squared_plot, ncol = 2)
